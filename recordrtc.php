...
if ( $action != '' && isset($SESSION->recordrtc_bbbsession) && !is_null($SESSION->recordrtc_bbbsession) ) {
    ...
    $bbbsession['bbb_endpoint'] = recordrtc_get_cfg_server_url();
    $bbbsession['bbb_secret'] = recordrtc_get_cfg_shared_secret();
    ...
    $bbbsession['meetingid'] = sha1($CFG->wwwroot.$cm->id.$USER->id.$bbbsession['bbb_secret']);
    if ( isset($cm->modname) ) {
    ...
    $bbbsession['originTag'] = 'moodle-atto_recordrtc ('.get_config('mod_recordrtc', 'version').')';
    // Metadata (context)
    ...
    // Operation URLs
    $sesskey = sesskey();
    $bbbsession['logoutURL'] = $CFG->wwwroot.MOODLE_ATTO_RECORDRTC_URL.'?contextid='.$contextid.'&content='.urlencode($content).'&sesskey='.$sesskey.'&action=logout';
    $bbbsession['launchURL'] = $CFG->wwwroot.MOODLE_ATTO_RECORDRTC_URL.'?contextid='.$contextid.'&content='.urlencode($content).'&sesskey='.$sesskey.'&action=launch';
...
$recordings = recordrtc_getRecordingsArray( $bbbsession['meetingid'], $bbbsession['bbb_endpoint'], $bbbsession['bbb_secret'] );
...
if ( $action == 'launch' || $action != 'logout' && !recordrtc_is_annotated($content) && empty($recordings) ) {
    ...
    $error = array();
    $meeting = recordrtc_getMeetingArray( $bbbsession['meetingid'], $bbbsession['bbb_endpoint'], $bbbsession['bbb_secret'] );
    if ( $meeting ) {
        ...
        $response = recordrtc_getCreateMeetingArray(
                $bbbsession['meetingname'],
    ...
    if ( empty($error) ) {
        ...
        $defaultConfigXML = recordrtc_getDefaultConfigXML($bbbsession['bbb_endpoint'], $bbbsession['bbb_secret']);
        ...
        // obtain join_url
        $join_url = recordrtc_getJoinURL($bbbsession['meetingid'], $bbbsession['username'], $password, $bbbsession['bbb_secret'], $bbbsession['bbb_endpoint'], $bbbsession['logoutURL'], $configToken);
        // render javascript for doing the redirect
    ...
    $rec_img_src = $CFG->wwwroot.'/mod/recordrtc/pix/bigbluebutton-80.png';
    $rec_img_lbl = get_string('recording_ready', 'atto_recordrtc');
    foreach ( $recordings as $recording ) {
        // If recording is the one used in the annotation add class 'active'
        $rec_lnk_att = array('onclick' => 'M.atto_recordrtc.view_select(this);', 'type' => 'button', 'class' => 'btn btn-secondary recording', 'ondblclick' => 'window.open(\''.$recording['playbacks']['presentation']['url'].'\');return false;', 'id' => $recording['recordID'], 'title' => $rec_img_lbl, 'data-url' => $recording['playbacks']['presentation']['url']);
        $rec_btn =  html_writer::tag('button', html_writer::img($rec_img_src, $rec_img_lbl), $rec_lnk_att);
        $out .= '<div id="'.$recording['recordID'].'" class="show-recording">'.$rec_btn.'<input class="delete" type="button" value="x" onclick="M.atto_recordrtc.view_delete(\''.$recording['recordID'].'\');" /></div>';
    }
    ...
    if ( $action == 'logout' ) {
        ...
        $poll_img_lbl =get_string('recording_processing', 'atto_recordrtc');
        $poll_img_att = array('id' => 'recording_polling', 'title' => get_string('recording_processing', 'atto_recordrtc'));
    ...
    // Button for Annotate
    $text = get_string('action_annotation_annotate', 'atto_recordrtc');
    $linkatt = array('onclick' => 'M.atto_recordrtc.view_annotate();', 'class' => 'hidden-print');
    $button =  html_writer::tag('button', $text, $linkatt);
    $out .= '<br><br>'.$button;
    // Button for Cancel
    $text = get_string('action_annotation_cancel', 'atto_recordrtc');
    $linkatt = array('onclick' => 'M.atto_recordrtc.view_cancel();', 'class' => 'hidden-print');
    $button =  html_writer::tag('button', $text, $linkatt);
    $out .= '&nbsp;'.$button;
    $out .= "\n".'</center></div>'."\n";
    ...
    $jsVars = array(
      ...
      'action' => $action,
      'recording_ready' => get_string('recording_ready', 'atto_recordrtc'),
      'recording_icon80' => $CFG->wwwroot.'/mod/recordrtc/pix/bigbluebutton-80.png',
      'recording_icon32' => $CFG->wwwroot.'/mod/recordrtc/pix/bigbluebutton-32.png'
    );
    $PAGE->requires->data_for_js('recordrtc', $jsVars);

    $jsmodule = array(
        'name'     => 'atto_recordrtc',
        'fullpath' => MOODLE_ATTO_RECORDRTC_ROOT.'atto_module.js',
        'requires' => array('datasource-get', 'datasource-jsonschema', 'datasource-polling'),
    );
    $PAGE->requires->js_init_call('M.atto_recordrtc.view_init', array(), false, $jsmodule);
}
