 ...
 */
 ...
 M.atto_recordrtc.datasource_init = function(Y) {
     recordrtc_dataSource = new Y.DataSource.Get({
         source : M.cfg.wwwroot + "/mod/recordrtc/bbb_broker.php?"
     });
 };

M.atto_recordrtc.view_init = function(Y) {
    console.info('Init...');

    // Init general datasource
    M.atto_recordrtc.datasource_init(Y);
...
M.atto_recordrtc.view_annotate = function() {
    console.info('Annotate...');
    var button = window.parent.Y.namespace('M.atto_recordrtc').atto_recordrtc_button;
...
M.atto_recordrtc.view_cancel = function() {
    console.info('Cancel...');
    var button = window.parent.Y.namespace('M.atto_recordrtc').atto_recordrtc_button;
    button._toggle_action();
};

M.atto_recordrtc.view_select = function(b) {
    console.info('Select...');
...
M.atto_recordrtc.view_delete = function(record_id) {
    console.info('Delete...');
...
_update_recording_polling = function(recording) {
    ...
    var div_new_recording = Y.Node.create('<div id="' + recording['recordID'] + '" class="show-recording"><input class="delete" type="button" value="x" onclick="M.atto_recordrtc.view_delete(\'' + recording['recordID'] + '\');" /></div>');
    var new_recording = Y.Node.create('<button onclick="M.atto_recordrtc.view_select(this);" type="button" class="btn btn-secondary recording" ondblclick="window.open(\''+recording['playbacks']['presentation']['url']+'\');return false;" id="' + recording['recordID'] + '" title="' + recordrtc.recording_ready + '" data-url="' + recording['playbacks']['presentation']['url'] + '"><img src="' + recordrtc.recording_icon80 + '" alt="Recording ready" /></button>');
